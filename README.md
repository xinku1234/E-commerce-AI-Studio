...
| 模块 | 当前状态 | 说明 |
| --- | --- | --- |
| 商品与视觉分析 | 可用 | 必须绑定模型；调用失败会返回真实错误，不会静默降级为本地结果 |
| 商品图片生成 | 可用 | 取决于模型、账号权限和接口兼容性；调用失败返回 `502 IMAGE_GENERATION_FAILED`，界面标记为「生成失败」 |
| 详情页生成 | 可用 | 必须绑定模型；仅 `REQUIRE_MODEL=false` 演示模式返回本地模板并显式标注 |
| 批量工作台 | 可用 | 逐项调用图片 Provider；本地合成结果标记为「本地合成」并写入导出元数据，任务元数据可跨刷新恢复 |
...
- 图片生成 Provider 使用统一结果协议，返回实际 provider、模型、真实 AI 状态和失败原因。
- 生图调用失败时不会回退本地画布：接口返回 `502 IMAGE_GENERATION_FAILED`，五图矩阵对应槽位保持为空并显示失败原因。
...
生成完成后，前端会重新检查最终成品，而不仅检查输入参考图。五图套装会记录每张图的质量评分、来源（模型生成 / 本地合成）和人工复核提示；Amazon 或白底槽位还会额外检查纯白背景覆盖率。质量评分只在模型真实返回图片后展示，失败槽位不会显示评分或成功标记。

每张图片最多调用模型两次。成品评分明显过低时，会带着更严格的构图和一致性约束重试一次，并保留评分更高的版本。如果两次都失败，该槽位标记为失败并展示接口返回的真实原因，不会用本地合成图占位，也不会计入「已生成」数量或打包导出。
